...
def compute_uncertainty_batch(
    ...
    input_images = input_images.expand(
        n_models, bsize, model.opt.ncond, 3, model.opt.height, model.opt.width
    )
    ...
    input_images = input_images.view(
        bsize * n_models, model.opt.ncond, 3, model.opt.height, model.opt.width
    )
    ...
    if hasattr(model, "cost"):
        pred_costs = model.cost(
            pred_images.view(-1, 3, 117, 24), pred_states.data.view(-1, 4)
        )
        pred_costs = pred_costs.view(n_models, bsize, npred, 2)
        pred_costs = (
            pred_costs[:, :, :, 0] + model.opt.lambda_l * pred_costs[:, :, :, 1]
        )
        if detach:
            pred_costs.detach_()
    else:
    ...
    pred_images = pred_images.view(
        n_models * bsize, npred, 3, model.opt.height, model.opt.width
    )
...
def plan_actions_backprop(
    ...
    if use_action_buffer:
        actions = torch.cat(
            (
                model.actions_buffer[nexec:, :],
                torch.zeros(nexec, model.opt.n_actions).cuda(),
            ),
            0,
        ).cuda()
    ...
    input_images = input_images.expand(
        n_futures, model.opt.ncond, 3, model.opt.height, model.opt.width
    )
    ...
    gamma_mask = (
        torch.from_numpy(numpy.array([0.99**t for t in range(npred + 1)]))
        .float()
        .cuda()
        .unsqueeze(0)
        .expand(n_futures, npred + 1)
    )
    ...
    for i in range(bprop_niter):
        ...
        pred, _ = model.forward(
            [input_images, input_states], actions_rep, None, sampling="fp", z_seq=Z
        )
        ...
        if hasattr(model, "value_function"):
            v = model.value_function(
                pred[0][:, -model.value_function.opt.ncond :].contiguous(),
        ...
        offroad_loss = torch.mean(
            utils.offroad_cost(pred_images, prox_map_l) * gamma_mask[:, :npred]
        )
        ...
        loss.backward()
        print(
            "[iter {} | mean pred cost = {:.4f}, uncertainty = {:.4f}, grad = {}".format(
                i,
                proximity_loss.item(),
                uncertainty_loss.item(),
                actions.grad.data.norm(),
            )
        )
...
def get_goal(current_position: torch.Tensor, goal_list: torch.Tensor) -> torch.Tensor:
    """
    current_position: B x 2
    goal_list: B x Variable x 2
...
def train_policy_net_mpur(
    ...
    for t in range(npred):
        ...
        if index % 100 == 0 and t == 0:
            viz_image = visualize_goal_input(
                input_images, current_goal, s_std=model.stats["s_std"]
            )
            plt.imshow(viz_image)
            wandb.log({f"train/goal_viz": plt}, step=index)
    ...
    if n_updates_z > 0:
        ...
        for k in range(n_updates_z + 1):
            ...
            pred_cost_adv, _ = utils.proximity_cost(
                pred[0],
                pred[1].data,
                car_sizes,
                unnormalize=True,
            ...
            if k < n_updates_z + 1:
                ...
                print(
                    f"[z opt | iter: {k} | pred cost: {pred_cost_adv.mean().item()} | u_cost: {total_u_loss.mean().item()}]"
                )
    ...
    if not hasattr(model, "cost"):
        ...
        offroad_cost = utils.offroad_cost(
            pred_images[:, :, :3].contiguous(), prox_map_l
        )
        if hasattr(model, "value_function"):
            v = model.value_function(
                pred_images[:, -model.value_function.opt.ncond :, :3].contiguous(),
    ...
    predictions = dict(
        state_img=(
            pred_images + input_ego_car_orig[:, None].expand_as(pred_images)
        ).clamp(max=1.0),
...
def get_grad_vid(model, input_images, input_states, car_sizes, device="cuda"):
    ...
    loss = proximity_loss + opt.lambda_l * lane_loss
    loss.backward()

    return input_images.grad[:, -1, :3].abs().clamp(max=1.0)


def train_policy_net_mper(
    model, inputs, targets, targetprop=0, dropout=0.0, n_models=10, model_type="vae"
):
    ...
    for t in range(npred):
        ...
        if model_type == "ten" or model_type == "vae":
            ...
            z_ = z
            z_list.append(z_)
            z_exp = model.z_expander(z_).view(
                bsize, model.opt.nfeature, model.opt.h_height, model.opt.h_width
            )
            h_x = h_x.view(
                bsize, model.opt.nfeature, model.opt.h_height, model.opt.h_width
            )
