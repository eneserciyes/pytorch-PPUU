...
parser.add_argument(
    "--beta", type=float, default=0.0, help="weight coefficient of prior loss"
)
parser.add_argument(
    "--p_dropout", type=float, default=0.0, help="set z=0 with this probability"
)
...
parser.add_argument(
    "--mfile",
    type=str,
    default="model=fwd-cnn-layers=3-bsize=64-ncond=20-npred=20-lrt=0.0001-nfeature=256-dropout=0.1-gclip=5.0-warmstart=0-seed=1.step200000.model",
)
...
def compute_loss(targets, predictions, gamma=1.0, r=True):
    ...
    loss_i = (
        F.mse_loss(pred_images, target_images, reduce=False).mean(4).mean(3).mean(2)
    )
...
def train(nbatches, npred):
    ...
    for i in range(nbatches):
        ...
        pred, _ = planning.train_policy_net_mper(
            model, inputs, targets, dropout=opt.p_dropout, model_type=model_type
        )
...
def test(nbatches, npred):
    ...
    for i in range(nbatches):
        ...
        pred, pred_actions = planning.train_policy_net_mper(
            model,
            inputs,
            targets,
            targetprop=opt.targetprop,
            dropout=0.0,
...
if opt.test_only == 1:
    ...
    wandb.init(project="mper-ppuu")
    print("[training]")
    print(f"[job name: {opt.model_file}]")
    ...
    for i in range(500):
        ...
        wandb.log(
            {
                "Loss/train_state_img": train_losses[0],
                "Loss/train_state_vct": train_losses[1],
                "Loss/train_costs": train_losses[2],
                "Loss/train_policy": train_losses[3],
                "Loss/train_relative_entropy": train_losses[4],
                "Loss/validation_state_img": valid_losses[0],
                "Loss/validation_state_vct": valid_losses[1],
                "Loss/validation_costs": valid_losses[2],
                "Loss/validation_policy": valid_losses[3],
                "Loss/validation_relative_entropy": valid_losses[4],
            },
            step=i,
        )
        ...
        log_string = (
            f"step {n_iter} | npred {npred} | bsize {bsize} | esize {opt.epoch_size} | "
        )
        log_string += utils.format_losses(
            train_losses[0], train_losses[1], split="train"
        )
        log_string += utils.format_losses(
            valid_losses[0], valid_losses[1], split="valid"
        )
        print(log_string)
        # utils.log(opt.model_file + '.log', log_string)
        if (
            i > 0
            and (i % opt.curriculum_length == 0)
            and (opt.npred == -1)
            and npred < 400
        ):
            npred += 8
