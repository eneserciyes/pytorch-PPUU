...
if "-ten" in opt.mfile:
    ...
    if os.path.isfile(pzfile):
        ...
        torch.save(
            {"knn_dist": model.knn_dist, "knn_indx": model.knn_indx}, pzfile + ".graph"
        )
    print("[done]")
    ...
    with torch.no_grad():
        ...
        if i < 10 and opt.save_video:
            for b in range(opt.batch_size):
                ...
                utils.save_movie(
                    dirname_movie, targets[0][b], targets[1][b], targets[2][b]
                )
        ...
        for s in range(opt.n_samples):
            ...
            pred, _ = model(
                inputs, actions, targets, sampling=opt.sampling
            )  # return as many predictions as actions
            ...
            if i < 10 and s < 20 and opt.save_video:
                for b in range(opt.batch_size):
                    ...
                    utils.save_movie(
                        dirname_movie, pred[0][b], pred[1][b]
                    )  # , pred_[2][b])
            ...
            if i < 10 and s < 20 and opt.save_video:
                for b in range(opt.batch_size):
                    ...
                    utils.save_movie(
                        dirname_movie, pred_rot[0][b], pred_rot[1][b]
                    )  # , pred_perm[2][b])
            ...
            if s == 0:
                ...
                for b in range(opt.batch_size):
                    ...
                    print("[saving video: {}]".format(dirname_movie), end="\r")
                    utils.save_movie(
                        dirname_movie, pred_true_z[0][b], pred_true_z[1][b]
                    )  # , pred_true_z[2][b])
                ...
                for b in range(opt.batch_size):
                    ...
                    print("[saving video: {}]".format(dirname_movie), end="\r")
                    utils.save_movie(
                        dirname_movie, pred_true_z_rot[0][b], pred_true_z_rot[1][b]
                    )
